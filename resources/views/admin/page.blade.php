            ...
            <div class="col-xl-4 col-lg-4 col-md-4 w-100">
                <div class="p-3 mb-4" style="background-color: #EEEFE0">
                    ...
                    <div class="row justify-content-center">
                        <div class="col-md-6">
                            <a id="forward-you" href="/admin/complaints/forwarded?direction=to" target="_blank"
                        ...
                        <div class="col-md-6">
                            <a id="forward-unheard" href="/admin/complaints/not_opened" target="_blank"
        ...
        <script>
            $(document).ready(function() {
                    ...
                    success: function(data) {
                        ...
                        data.forEach(row => {
                            ...
                            const makeLink = (section, type, user) =>
                                `/admin/complaints/${section}?type=${type}&user=${user}`;
                                ...
                                <tr class="${rowColor}">
                                    ...
                                    <td style="white-space: nowrap;">
                                       <button class="btn btn-sm btn-primary view-details-btn" data-id="${row.complaint_id}">
                                         ...
                                         <a href="/admin/details_suchnas/${row.complaint_id}" 
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.length === 0) {
                            ...
                            data.forEach(item => {
                                $list.append(`
                                  <a href="/admin/complaints/vibhag-details?department=${item.department}" target="_blank" class="text-decoration-none" style="color: black">
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.length === 0) {
                                ...
                                data.forEach(item => {
                                    $list.append(`
                                        <a href="/admin/complaints/status-details?status=${encodeURIComponent(item.status)}&filter=${filter}" target="_blank" class="text-decoration-none" style="color: black">
                                            <li class="d-flex justify-content-between border-bottom py-1">
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.length === 0) {
                                ...
                                data.forEach(item => {
                                    $list.append(`
                                        <a href="/admin/complaints/suchna-status-details?status=${encodeURIComponent(item.status)}&filter=${filter}" target="_blank" class="text-decoration-none" style="color: black">
                                            <li class="d-flex justify-content-between border-bottom py-1">
                                                <span>${item.status}</span>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('.new-voters').html(
                            `<a href="/admin/voters/details?filter=today-voters" target="_blank" class="text-decoration-none" style="color: black">
                                <i class="fa fa-users"></i> ${data.new_voters}
                            </a>`
                        );

                        $('.new-contacts').html(
                            `<a href="/admin/voters/details?filter=today-contacts" target="_blank" class="text-decoration-none" style="color: black">
                                <i class="fa fa-phone"></i> ${data.new_contacts}
                            </a>`
                        );

                        $('.total-voters').html(
                            `<a href="/admin/voters/details?filter=total-voters" target="_blank" class="text-decoration-none" style="color: black">
                                <i class="fa fa-check-square"></i> ${data.total_voters}
                            </a>`
                        );

                        $('.total-contacts').html(
                            `<a href="/admin/voters/details?filter=total-contacts" target="_blank" class="text-decoration-none" style="color: black">
                ...
                function fetchForwardedCounts() {
                    $.ajax({
                        url: "{{ route('adminajax.forwarded.counts') }}",
                        method: 'GET',
                        dataType: 'json',
                        success: function(data) {
                ...
                function fetchForwardedCountsVikash() {
                    $.ajax({
                        url: "{{ route('adminajax.forwarded.counts.vikash') }}",
                        method: 'GET',
                        dataType: 'json',
                        success: function(data) {
                            ...
                            data.forEach(item => {
                                $table.append(`
                                    <tr>
                                        ...
                                        <td>
                                            <a href="/admin/complaints/forward-details?forward=${item.forward}&type=शुभ सुचना" target="_blank" class="text-decoration-none text-light">
                                                <span class="badge bg-success border" style="color: white">${item.subh}</span>
                                            </a>
                                        </td>
                                        <td>
                                            <a href="/admin/complaints/forward-details?forward=${item.forward}&type=अशुभ सुचना" target="_blank" class="text-decoration-none text-dark">
                                                <span class="badge bg-danger border" style="color: white">${item.asubh}</span>
                                            </a>
                                        </td>
                                        <td>
                                            <a href="/admin/complaints/forward-details?forward=${item.forward}&type=समस्या" target="_blank" class="text-decoration-none text-dark">
                                                <span class="badge bg-warning text-dark border">${item.samasya}</span>
                                            </a>
                                        </td>
                                        <td>
                                            <a href="/admin/complaints/forward-details?forward=${item.forward}&type=विकास" target="_blank" class="text-decoration-none text-dark">
                ...
                function getFollowupUrl(status, filter, type, operator) {
                    let baseUrl = "{{ route('admincomplaints.followupDetails') }}";
                ...
                function getNotdoneFollowupUrl(status, type) {
                    let baseUrl = "{{ route('admindashboard.notDoneDetails') }}";
                ...
                function loadAllFollowupCounts(filter = 'कुल', operator = 'सभी') {
                    $.when(
                        $.ajax({
                            url: "{{ route('admindashboard.followupCounts') }}",
                            method: "GET",
                            data: {
                                filter: filter,
                                operator: operator
                        ...
                        $.ajax({
                            url: "{{ route('admindashboard.notDoneCounts') }}",
                            method: "GET",
                            data: {
                                filter: filter,
                                operator: operator
                ...
                $.ajax({
                    url: "/admin/ajax/complaints/not_opened_count",
                    type: "GET",
                    success: function(data) {
                ...
                $.ajax({
                    url: "/admin/ajax/complaints/not_opened_count_vikash",
                    type: "GET",
                    success: function(data) {
                ...
                for (let i = 0; i < 6; i++) {
                    for (let j = 0; j < 7; j++) {
                        if (i === 0 && j < firstDay) {
                            ...
                            table += `<td class="p-1">
                                <a href="/admin/complaints/date-wise?date=${fullDate}" target="_blank" class="text-decoration-none text-dark">
